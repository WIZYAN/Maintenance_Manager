...
## 调试时将两路起点重新设为今天（一次性操作）

用于已保存早期调试日期、希望重新开始当前一轮的场景；不会自动判断或覆盖历史起点。

1. 重新编译、下载 MCU 程序，运行到能正常读取屏幕 RTC，确认日期正确。
2. 暂停程序，在 e² studio 的 Expressions 中将 `Maintenance_para.reset_all_request` 的值改为 `1`。
3. 恢复运行。主循环等待有效 RTC 和可用存储，然后将整机与传感器起点同时改为当天，用一次 EEPROM 双备份提交保存；两路周期保持原值。
4. 请求消费后 `reset_all_request` 自动变为 `0`。查看 `reset_all_result`：`MAINTENANCE_OK`（0）表示回读校验成功；180 天周期此时显示 `0/180`。

请求每次开机默认关闭，重启继续按保存起点累计。不要将其初始值改为 `1`，也不要在循环中反复置位。
写入失败会记录结果并停止本次操作；恢复存储后如仍需重置，再手动请求。提交后最终回读失败时，重载可能发现新记录已经生效，应先检查起点。
两份记录均损坏时，手动请求会使用默认周期重建两路记录，与原有 Reset 接口的恢复规则一致。
...
| 7、8、9 | 原有说明及售后电话文本 | 保留原文，调整字号、颜色与位置，MCU 不写入 |
| 10 | 屏幕 RTC 日期时间 | 调整字号、颜色与位置，保持 RTC 行为 |
...
`VisualTFT/Project/Screen0.tft` 使用 1024×600 原生控件布局：炭灰底（32,35,38）、石墨面板（45,50,55）、橙色数字/进度（255,169,77）。
整机与传感器分为上下两块面板；分子使用 64 像素字高，分母 40 像素，说明文字 32 像素。
数字区域按最大五位数周期及斜杠预留宽度。初始占位仍是 `--/--`，进度初值为 0。
新增静态标题控件 11、原生矩形装饰 12～16；原控件 1～10 的编号、输入行为及 MCU 更新方式不变。
页面没有新增 Lua 或业务逻辑，也不依赖新增背景位图。屏幕资源已编译，实屏字体与刷新效果待下载后确认。
